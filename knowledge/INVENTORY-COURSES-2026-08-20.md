...
> נמצאו **3 סטיות אמיתיות** בין משטחי-הריבוי + פער-לגאסי + 4 קלים (§9).
> **סטטוס 20.8: S1·S2·S3·S4·S7 תוקנו/אומתו; S5·S6 נבדקו ונשארו במכוון.** ‏1904 בדיקות + 2 סוויטות-דפדפן ירוקות.
...
### ✅ S3 · פער-לגאסי — **נסגר ואומת 20.8**
היה: `AUDIT-SIGNUP-2026-07-31.md:35` — שיבוץ-כרטיסייה **מכרטיס-המשפחה** ב-demo-walkthrough לא השלים. היה נגזרת של S1/S2 (שיבוץ + ניקוב מכרטיס-המשפחה).
**אימות:** אחרי S1+S2 — `node e2e/demo-walkthrough.mjs` = **100/100**, כולל "✅ שיבוץ כרטיסייה מכרטיס המשפחה" + "✅ ניקוב מכרטיס המשפחה — היתרה ירדה". גם `launch-readiness` 14/14.

### 🟢 קלים (קוסמטי/מתועד)
- **✅ S4 · תוקן 20.8** — היה: `planLabelOf` מוצג פעמיים (עמודות "מסלול"+"יתרה") כשהניקוב כבוי. **תיקון:** עמודת-היתרה עברה ל-`punchOn && isPunch ? bar : presentsInMonth` (בלי ענף-כפילות). ratchet `inventory-cleanup.test.ts`.
- **✅ S7 · תוקן 20.8** — `nextSessionDate(c, now = new Date())` — `now` מוזרק ⇒ דטרמיניסטי ובר-בדיקה (התנהגות ברירת-מחדל זהה). ratchet `inventory-cleanup.test.ts`.
- **⏸ S5 · נבדק — אין שינוי (החלטה מתועדת).** תצוגת-quote פר-שיעור: ההערה `EnrollModal:362` מבהירה במכוון ששני הנתונים (שיעורים/מחיר-לשיעור) מתוארים בלי לטעון שמכפלתם=הסה"כ (עיגול חצאי-שיעור). כבר משתמש ב-"·" ולא "×" — לא לעקוף החלטה מתועדת.
- **⏸ S6 · נבדק — shells בכוונה נפרדים.** `courses/AbsenceModal` ו-`diary/DiaryAbsenceModal` חולקים כבר את **כל הלוגיקה** (`makeupEligibility`+`ABSENCE_REASON_CHIPS`+דלתות-אמינות זהות; באג-הסטייה #7 נסגר). ההבדלים הם הצגה תלוית-הקשר (היומן מציג *מי*+שעת-המפגש הבא, באנר-למעלה). מיזוג-shells = סיכון-רגרסיה בשני משטחים-חיים בלי רווח-לוגי.
