...
### 🟠 S1 · שני מסלולי-שיבוץ שנפרדו (מאומת)
`EnrollModal` (כרטיס-חוג) ו-`JoinModal` (כרטיס-משפחה) שניהם יוצרים שיבוץ, אך:
`JoinModal.tsx:263` **מקבע `totalDue:0`** ואינו כותב שדות פר-שיעור. ⇒ שיבוץ אותו חוג-פר-שיעור משני המשטחים נותן **נתוני-תמחיר שונים** (מכרטיס-המשפחה = חוב-אפס, ללא quote משוקלל).
**תיקון-מוצע:** לחלץ בונה-שיבוץ משותף (או להחיל `enrollmentQuote`/`pricingFields` גם ב-JoinModal).

### 🟠 S2 · ניקוב מכרטיס-המשפחה עוקף את מנגנון-הנוכחות החדש (מאומת)
`FamilyPanels.tsx:259` — `doPunch` קורא ל-`punch(e.id)` הגולמי (ההערה :242 "אותו store.punch" מיושנת). מסך-החוג והיומן עברו ל-`setPresent` (אידמפוטנטי, מזין `presents[]`+מונה-חודשי). ⇒ ניקוב מכרטיס-המשפחה **לא מופיע ב"נוכחויות החודש"**, ואינו אידמפוטנטי (בדיוק מחלקת-הבאג #6/#10 שנסגרה במקומות האחרים).
**תיקון-מוצע:** להעביר את `FamilyPanels.doPunch` דרך `setPresent(e.id, isoToday(), true)` (כמו CourseDetail:159), + ratchet cross-surface.
