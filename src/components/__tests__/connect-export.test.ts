...
describe('🔌 ratchet — חיבור 6: תדפיסים וייצוא', () => {
  ...
  it('CSV מסירות (shop7): כותרת + שורה לכל מסירה — שטרם-נמסר מסומן ולא מוסתר', () => {
    ...
    // גל ב׳ (4.8.2026): נוספה עמודת 'כתובת' — דף-מסלול אמיתי (שדרוג-פורמט מודע)
    expect(rows[0]).toEqual(['תאריך', 'משפחה', 'כתובת', 'מתנדב', 'סטטוס', 'הערה']);
    expect(rows).toHaveLength(3); // כותרת + 2 מסירות (כולל "איסוף" שטרם נמסר)
    expect(rows).toContainEqual(['2026-07-20', 'כהן', 'הרצל 5, בני ברק', 'משה', 'נמסר', '']);
    expect(rows).toContainEqual(['2026-07-20', 'כהן', 'הרצל 5, בני ברק', 'משה', 'איסוף', 'קומה 3']);
  });
