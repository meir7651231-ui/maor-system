...
export function SupportersGalaxy(props: {
  ...
  const nodes = useMemo(
    () => donorConstellation(props.supporters, today, { rate }),
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      for (const b of [0.3, 0.5, 0.7, 0.9]) { ctx.beginPath(); ctx.arc(cx, cy, maxR * b, 0, 6.2832); ctx.stroke(); }
      // core
      ...
      for (const n of shown) {
        ...
        const y = cy + Math.sin(ang) * n.radius * maxR;
        const sz = 2 + n.size * 6;
