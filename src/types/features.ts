...
export const FEATURES: FeatureDef[] = [
  ...
  { key: 'courses.reenroll', label: 'רישום לשנה הבאה', desc: 'מסך "רישום" עם חייגן — סיכום מה היה בחוג פר-תלמיד/ה, החלטת המשך (ממשיך/בהמתנה/עוזב), ורישום לשנה הבאה ששומר היסטוריה. אופט-אין (מכובה כברירת-מחדל).', module: 'courses', optIn: true },
  ...
  { key: 'courses.reenroll.familypanel', label: 'רישום לשנה הבאה בכרטיס-המשפחה', desc: 'פאנל בכרטיס-המשפחה: החלטת-המשך + היסטוריית-תלמיד + רישום פר-ילד/ה, ישירות מהכרטיס.', module: 'courses' },
  ...
  { key: 'courses.teacherapp', label: 'אפליקציית-המורה', desc: '🎓 מסך מצומצם למורה (opt-in) — המפגשים-שלי-היום + נוכחות-בטאפ + השלמות-ממתינות + דוח-חודשי; רק לתפקיד-מורה, אפס-כסף', module: 'courses', optIn: true },
  ...
  { key: 'supporters.prayernames', label: 'שמות לתפילה 🙏', desc: 'רשימת שמות-לתפילה עם הערה לכל שם בכרטיס-התורם — הוספה, עריכת-הערה, סימון, העתקה (בקשת-בעלים 5.9)', module: 'supporters' },
  ...
  { key: 'supporters.ayin.unassignondone', label: 'ביטול-ייעוד בהשלמה', desc: 'כשתיק-טיפול מגיע ל"הושלם" — הייעוד (forWho) של התומך/ת מתבטל אוטומטית ⇒ העובד/ת לא רואה אותו יותר, רק המנהל, עד שהמנהל מייעד מחדש (opt-in). כבוי=ביט-זהה', module: 'supporters', optIn: true },
  ...
  { key: 'shell.pwa', label: 'התקנה כאפליקציה 📱', desc: 'האתר ניתן להתקנה במסך-הבית (PWA) — אייקון משלו, מסך-מלא ועבודה גם בלי רשת; כבוי = אתר רגיל בדפדפן', module: 'shell' },
  { key: 'shell.update', label: 'באנר "יש גרסה חדשה" 🔄', desc: 'כשיוצאת גרסה חדשה מוצג באנר עדין למעלה עם "רענן עכשיו" — הלקוח יודע שיש עדכון ורואה אותו בלי לרענן ידנית; כבוי = בלי בדיקת-גרסה', module: 'shell' },
  { key: 'shell.publicsite', label: 'אתר ציבורי 🌐', desc: 'דף-נחיתה ציבורי (בלי התחברות) שמוזן ישירות מהקונפיג של הארגון — נפתח בכתובת ‎?site‎; מיתוג, קמפיין, שירותים, סיפור, גלריה וקשר. כבוי = אין דף ציבורי', module: 'shell' },
  { key: 'shell.portal', label: 'שער-הצטרפות באתר 🚪', desc: '🚪 מרכז-כניסה בעמוד-השיווק (opt-in): 👪 הרשמת ילד/ה לחוג (טופס ציבורי → שליחה רב-ערוצית WhatsApp/SMS/מייל/טלפון) + 🔑 כניסת-צוות. אפס-שרת, נוסף לצד הכניסה הקיימת', module: 'shell', optIn: true },
  { key: 'shell.teamchat', label: 'צ׳אט צוות 💬', desc: 'צ׳אט פנימי חי בין אנשי-הצוות של הארגון (מנהל+עובדות) — נפתח מ-❓ ומסתנכרן בזמן-אמת בין כל המכשירים. דורש ענן+התחברות. כבוי = אין צ׳אט-צוות', module: 'shell' },
  { key: 'reports.ayin', label: 'דוח שמות-לטיפול 👁', desc: 'פילוח שם-לטיפול + כמות במסך הדוחות (מכל התיקים) עם ייצוא-CSV; פעיל רק כשמעקב-הטיפול דלוק', module: 'reports' },
  ...
  // ——— פירוק-יכולות 20.8 (flag-max 2, בקשת-בעלים "מלא מלא כפתורים") ———
  // חוזה: חסר = פעיל (ביט-זהה ללקוחות חיים); false מכבה. הגידור בקומפוננטות.
  // — חנות —
  { key: 'shop.items', label: 'פריטי קטלוג', desc: 'פאנל "📦 פריטים" — ניהול פריטים עצמאיים עם מלאי משותף', module: 'shop' },
  { key: 'shop.quickredeem', label: 'מימוש מהיר מהבית', desc: 'בוחר "🎁 מימוש מהיר" במסך-הבית של החנות — קיצור בלי לפתוח כרטיס', module: 'shop' },
  { key: 'shop.care', label: 'רצועת "מה מגיע וטרם נמסר"', desc: 'מקטע needsCare במסך-הבית של החנות + מימוש-בקליק', module: 'shop' },
  { key: 'shop.holidays', label: 'רצועת החגים הקרובים', desc: '"🕎 החגים הקרובים ומי מקבל מה" במסך-הבית של החנות', module: 'shop' },
  { key: 'shop.assignstatus', label: 'שינוי סטטוס שיוך', desc: 'בורר ▶ פעיל / ✅ הושלם / ⏸ הופסק על שיוך (מזין את החלוקה)', module: 'shop' },
  { key: 'shop.showcase.wall', label: 'מסך-מלא לראווה', desc: 'כפתור "🖥 מסך מלא" — מצב-קיר למסך גדול', module: 'shop' },
  { key: 'shop.calendar.hebtoggle', label: 'מתג עברי/לועזי בלוח החנות', desc: 'כפתור "ללועזי / לעברי" בלוח הייעודי', module: 'shop' },
  { key: 'shop.histfilter', label: 'מסנן היסטוריית מימושים', desc: 'טווח-תאריכים + "כולל מבוטלים" מעל היסטוריית-המימושים', module: 'shop' },
  ...
  { key: 'tzedaka.care', label: 'רצועת "דורש טיפול"', desc: 'קופות-רדומות/אבודות/מבצע במסך-הבית של הקופות', module: 'tzedaka' },
  { key: 'tzedaka.showcase.wall', label: 'מסך-מלא לראווה', desc: 'כפתור "🖥 מסך מלא" — מצב-קיר עם לוח-מובילים', module: 'tzedaka' },
  { key: 'tzedaka.calendar.hebtoggle', label: 'מתג עברי/לועזי בלוח הקופות', desc: 'כפתור "ללועזי / לעברי" בלוח הייעודי', module: 'tzedaka' },
  ...
  { key: 'shop7.dayclose', label: 'סגירת יום-חלוקה', desc: 'כפתור "סגור/פתח" על יום — נועל שיוכי-מסירה חדשים', module: 'shop7' },
  { key: 'shop7.note', label: 'הערת-מסירה חופשית', desc: 'שדה-הערה פר-מסירה בלוח (טקסט חופשי על מוטבים)', module: 'shop7' },
  // — תורמים —
  ...
  { key: 'supporters.bulkmail', label: 'שליחת-מייל מרוכזת 📧', desc: 'כפתור "📧 מייל למסומנים" במצב-הבחירה — שולח את אותו נוסח לכל התומכ/ות המסומנים בעלי מייל (דדופ לפי-כתובת); דורש הרחבת-מייל+ענן. מנהל/בעלים תמיד; לעובד/ת — המנהל מדליק', module: 'supporters' },
  { key: 'supporters.bulkwa', label: 'שליחת-וואטסאפ מרוכזת 💬', desc: 'כפתור "💬 וואטסאפ למסומנים" במצב-הבחירה — פותח רשימת wa.me פר-נמען (הודעה-אחת פר-טלפון); המזכירה לוחצת "פתח" פר-שורה. דורש הרחבת-וואטסאפ. מנהל/בעלים תמיד; לעובד/ת — המנהל מדליק', module: 'supporters' },
  { key: 'supporters.segula', label: 'סגולת 40 יום', desc: 'כפתור "🕯 40 ימים" בכרטיס התורם — זריעת תזכורות-לוח מדורגות לזיווג (יום 1/7/21/35/40) מהיום. דלוק כברירת-מחדל בכל עמותה (מוסתר רק בוורטיקל מסחרי); הדגל אינו מסתיר אותו.', module: 'supporters' },
  ...
  { key: 'supporters.sort', label: 'מיון בלחיצה על כותרת', desc: 'כותרות-הטבלה ממיינות בלחיצה (14 עמודות)', module: 'supporters' },
  { key: 'supporters.doncats', label: 'צ׳יפי קטגוריות בתרומה', desc: '5 הקטגוריות הנפוצות כצ׳יפים ברישום-תרומה', module: 'supporters' },
  { key: 'supporters.hint', label: 'שורת העזרה התחתונה', desc: 'פסקת 💡 הרמזים בתחתית מסך-התורמים', module: 'supporters' },
  { key: 'supporters.dialer.edit', label: 'עריכת פרטים מהחייגן', desc: 'כפתור "✎ עריכת פרטים" בתוך שיחת-חייגן', module: 'supporters' },
  { key: 'supporters.dialer.export', label: 'ייצוא יומן-חייגן', desc: 'כפתורי ⬇ CSV של יומן-הקמפיין (מכבד גם core.export)', module: 'supporters' },
  { key: 'supporters.dialer.namesexport', label: 'ייצוא שמות-הקמפיין', desc: 'כפתור ⬇ 🕯 — שמות-לטיפול של משתתפי-הקמפיין בלבד', module: 'supporters' },
  { key: 'supporters.dialer.reset', label: 'איפוס/ביטול בחייגן', desc: '"🗑 סיום קמפיין" (חמוש) + "↩ ביטול אחרון"', module: 'supporters' },
  { key: 'supporters.ayin.names', label: 'מסך-השמות המלא', desc: 'הטבלה החיה של כל השמות-לטיפול (חיפוש/סטטוס/שלב)', module: 'supporters' },
  { key: 'supporters.ayin.restart', label: 'מחזור טיפול חדש', desc: 'כפתור "↻ מחזור חדש" — פתיחת סבב מחדש (ההיסטוריה נשמרת)', module: 'supporters' },
  { key: 'supporters.ayin.nexttalk', label: '"מתי לדבר שוב"', desc: 'תאריך+שעה + 🔁 שוב — תזכורת-לוח מתוך כרטיס-הטיפול', module: 'supporters' },
  { key: 'supporters.ayin.answers', label: 'יומן תשובות', desc: 'רישום תשובות/הערות עם עריכה ומחיקה פר-שורה', module: 'supporters' },
  { key: 'supporters.ayin.revert', label: 'חזרה לשלב קודם', desc: 'לחיצה על גלולת-שלב מוקדמת מחזירה את התיק אחורה', module: 'supporters' },
  { key: 'supporters.doncal.hebgrid', label: 'גריד עברי בלוח-התרומות', desc: 'מתג עברי/לועזי בלוח-התרומות האישי', module: 'supporters' },
  { key: 'supporters.intel.signals', label: 'לוח-האותות (מודיעין)', desc: 'רצועת התזוזות-החריגות במרכז-המודיעין', module: 'supporters' },
  { key: 'supporters.intel.season', label: 'עונתיות (מודיעין)', desc: 'רצועת-העונתיות במרכז-המודיעין', module: 'supporters' },
  { key: 'supporters.intel.retention', label: 'שימור (מודיעין)', desc: 'רצועת-השימור במרכז-המודיעין', module: 'supporters' },
  { key: 'supporters.intel.pareto', label: 'פארטו (מודיעין)', desc: 'רצועת-הריכוזיות 80/20 במרכז-המודיעין', module: 'supporters' },
  { key: 'supporters.intel.time', label: 'ציר-זמן (מודיעין)', desc: 'רצועת ציר-הזמן במרכז-המודיעין', module: 'supporters' },
  { key: 'supporters.intel.cohort', label: 'קוהורטות (מודיעין)', desc: 'רצועת-הקוהורטות במרכז-המודיעין', module: 'supporters' },
  // — משפחות —
  { key: 'families.health', label: 'מידע רפואי/רגישויות ⚕', desc: 'שדה-הרגישויות בטופס, בכרטיס ובתדפיס-המורה (PII רפואי)', module: 'families' },
  { key: 'families.viewtoggle', label: 'מתג גריד/רשימה', desc: 'כפתור "☰ רשימה / ▦ גריד" במסך-המשפחות', module: 'families' },
  ...
  { key: 'families.sort', label: 'מיון בלחיצה על כותרת', desc: 'כותרות-הטבלה ממיינות בלחיצה', module: 'families' },
  // — חוגים —
  { key: 'courses.enroll.contact', label: 'רצועת פרטי-קשר בשיבוץ', desc: 'טלפונים/גיל/כיתה/עיר + "👤 הצג כרטיס מלא" במודאל-הניהול', module: 'courses' },
  ...
  // — מסך הבית (flag-max 2) —
  { key: 'home.care.full', label: 'מרכז-הטיפול המלא', desc: 'כפתור "המסך המלא ←" בווידג׳ט דורש-טיפול — מודאל עם חיפוש וסינון', module: 'home' },
  { key: 'home.care.filters', label: 'סינון והיסטוריה בטיפול', desc: 'צ׳יפי-תגיות + "הצג שטופלו" בווידג׳ט דורש-טיפול', module: 'home' },
  { key: 'home.care.reset', label: 'איפוס סימוני-טופל', desc: 'כפתור-האיפוס הגורף (חימוש דו-לחיצה) בווידג׳ט הטיפול', module: 'home' },
  { key: 'home.quick.backup', label: 'גיבוי מהבית', desc: 'כפתור "⬇ גיבוי" בווידג׳ט הפעולות-המהירות', module: 'home' },
  { key: 'home.hero.actions', label: 'פעולות-מהירות ב-Hero', desc: 'שורת הקיצורים בכרטיס-הפתיחה (ניקוב-מהיר, מי-חוגג ועוד)', module: 'home' },
  // — לוח שנה (flag-max 2) —
  { key: 'calendar.eventextra', label: '"פרטים נוספים" באירוע', desc: 'קיפול מחיר/דחיפות/חדר/שיוך-משפחה בטופס-האירוע', module: 'calendar' },
  { key: 'calendar.calldone', label: 'כפתור "✓ בוצע" לשיחות', desc: 'סימון אירוע-שיחה כטופל מתוך הלוח', module: 'calendar' },
  { key: 'calendar.addevent', label: 'הוספת אירוע מהלוח', desc: 'כפתור "➕ הוספת אירוע" בכותרת-הלוח (כיבוי = לוח לקריאה)', module: 'calendar' },
  // — יומן חדרים (flag-max 2) —
  { key: 'diary.blocking', label: 'חסימת שבת וחג', desc: 'חסימת-תזמון בשבתות/חגים ביומן-החדרים (ורטיקל מסחרי מכבה)', module: 'diary' },
  { key: 'diary.cleaning', label: 'משבצת ניקיון-יומי', desc: 'משבצת "ניקיון יומי 15:00–16:00" הקבועה בכל חדר', module: 'diary' },
  // — דוחות (flag-max 2) —
  { key: 'reports.colfilter', label: 'סינון עמודות בדוחות', desc: 'כפתור "⏷ סינון" ושורת-שדות פר-עמודה בכל טבלת-דוח', module: 'reports' },
  { key: 'reports.sort', label: 'מיון בלחיצה בדוחות', desc: 'כותרות-עמודה ממיינות בכל טבלאות-הדוחות', module: 'reports' },
  { key: 'reports.printall', label: 'הדפסת כל הדוחות', desc: 'כפתור "🖨 הדפסת כל הדוחות" הגורף', module: 'reports' },
  { key: 'reports.drilldown', label: 'צלילה מהדוח לכרטיס', desc: 'שורת-דוח לחיצה שפותחת את כרטיס-המשפחה/תורם (PII)', module: 'reports' },
  { key: 'reports.range', label: 'טווח-תאריכים לדוחות', desc: 'פאנל "🗓 טווח תאריכים" + קיצורי-הטווח', module: 'reports' },
  // — הגדרות (flag-max 2) —
  ...
  // — מעטפת (flag-max 2) —
  { key: 'shell.netcheck', label: 'בדיקת-תקשורת', desc: 'כלי 🩺 האבחון בתפריט-העזרה', module: 'shell' },
  { key: 'shell.support', label: 'צ׳אט-תמיכה', desc: '"💬 צ׳אט עם התמיכה" בתפריט-העזרה (דורש ענן)', module: 'shell' },
  ...
  { key: 'home.mytasks', label: '"המשימות שלי" 🎯', desc: 'התור שהמנהל הכין לעובד/ת — ווידג׳ט-בית עם עדיפויות, איחורים, קפיצה-לכרטיס ו-✓ בוצע', module: 'home' },
  { key: 'settings.workprep', label: 'מכין-העבודה (מנהל) 📋', desc: 'שיבוץ-משימות פר-עובד/ת מאשף ניהול-העובדות: משימה חופשית/קשורה-לתורם, עדיפות, יעד, ושיבוץ-המוני של יעדי-קשר שעברו', module: 'settings' },
];
