...
export interface Enrollment {
  ...
  year?: string;
  /**
   * 💰 יתרת-חוב שהובאה מהשנה הקודמת (בקשת-בעלים 25.8 "אם יש לו יתרה בשנה
   * הקודמת זה עובר לשנה הבאה"). ‏reenrollEnrollment מזין את payBal של-המקור
   * לתוך השדה — מוצג במאזן/סינון "לתשלום" יחד עם totalDue של-השנה-החדשה,
   * בלי לגעת ב-payments/receipts (הקבלה של-אשתקד נשארת אצל אותו שיבוץ ישן).
   * Additive · חסר/undefined = 0 (ביט-זהה לשיבוץ ישן). מסתנכרן כשדה-כמו-שאר
   * שדות-השיבוץ (upsertEnrollment → cloudSync ⇒ round-trip בין מכשירים).
   */
  carryBalance?: number;
}
