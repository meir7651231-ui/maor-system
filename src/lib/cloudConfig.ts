...
export interface EmployeeOverride {
  ...
  designations?: string[];
  /**
   * שם-תצוגה לעובד/ת (בקשת-בעלים 6.9 "בכל מייל בעובדים לרשום שם — שיראו בחוץ מי זה"):
   * מוצג במקום המייל בלוג-הפעולות, בצ׳אט-הצוות ובכרטיס-העובד. יושב ב-memberConfigs ⇒
   * Rules v3 (מנהל מעדכן memberConfigs) מכסים אותו בלי שינוי-Rules. חסר ⇒ המייל.
   */
  displayName?: string;
}
...
export const ORG_SECRET_KEYS = ['yemotToken', 'nedarimMosad', 'nedarimApiPass', 'smsApiKey', 'smtpUrl', 'solaXKey', 'gcontactsClientId', 'gcontactsClientSecret', 'gcontactsRefresh'] as const;
