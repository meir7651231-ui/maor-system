...
describe('🔎 ratchet — סיומת-ריבוי ולוונשטיין לא מזהמים חיפוש (פאס-4)', () => {
  it('שם קצר בסיומת ם (חיים/מרים) אינו מפעיל את התאמת סיומת-הריבוי', () => {
    // חיים→"חיימ" באורך 4: הגזע "חי" בן 2 היה מתאים "חיפה"/"חינוך" בציון 70.
    // אחרי התיקון (nq≥5) — אין התאמת גזע, וגם לוונשטיין(2)>max(1) → 0.
    expect(scoreTerm('חיים', 'חיפה')).toBe(0);
    expect(scoreTerm('מרים', 'מרכז')).toBe(0);
  });
  it('סיומת ריבוי אמיתית (5 תווים) עדיין מוצאת את הגזע', () => {
    expect(scoreTerm('חוגים', 'חוג')).toBe(70);
    expect(scoreTerm('קבלות', 'קבלה')).toBe(70);
  });
  it('מונח מספרי טהור (טלפון/ת.ז.) אינו מטושטש לרשומה שכנה', () => {
    // לפני: לוונשטיין(1)≤2 → 48 על מספר שונה בספרה. אחרי: מספר טהור מדולג.
    expect(scoreTerm('123456789', '123456780')).toBe(0);
    // חיפוש חלקי במספר עדיין עובד דרך prefix (80)
    expect(scoreTerm('12345', '123456780')).toBe(80);
  });
});
