...
describe('🕎 ratchet — כיסוי חגים מלא', () => {
  ...
  // דין-נדחה מלא (19.8) — שני הצומות הנותרים, מאומתים מול תאריכי-אמת:
  // ר"ה תשפ"ה חל ביום חמישי (3.10.2024) ⇒ ג' תשרי בשבת ⇒ הצום נדחה לראשון.
  it('צום גדליה שחל בשבת — נדחה ליום ראשון (2024)', () => {
    expect(holidayOf(noon('2024-10-05'))).toBeNull(); // שבת, ג' תשרי — הצום נדחה
    expect(holidayOf(noon('2024-10-06'))).toBe('צום גדליה (נדחה)'); // ראשון, ד' תשרי
  });
  // פורים תשפ"ד חל ביום ראשון (24.3.2024) ⇒ י"ג אדר ב' בשבת ⇒ התענית מוקדמת לחמישי.
  it('תענית אסתר שחלה בשבת — מוקדמת ליום חמישי (2024)', () => {
    expect(holidayOf(noon('2024-03-23'))).toBeNull(); // שבת, י"ג אדר ב' — אין תענית
    expect(holidayOf(noon('2024-03-21'))).toBe('תענית אסתר (מוקדם)'); // חמישי, י"א אדר ב'
    expect(holidayOf(noon('2024-03-24'))).toBe('פורים'); // ראשון — פורים כרגיל
  });
});
