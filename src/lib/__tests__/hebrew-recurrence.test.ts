...
describe('🏛️ קיר ההשפעה — יום-הולדת עברי ב"אדר" מופיע גם בשנה מעוברת (משטח רביעי)', () => {
  it('birthdaysToday מוצא נולד-אדר ב-אדר ב׳ מעוברת', async () => {
    const { birthdaysToday } = await import('../../components/wall/wallData');
    const birthIso = hebToIso(12, 'אדר', common)!;
    const leapAdar2 = hebToIso(12, 'אדר ב׳', leap)!;
    const db = {
      ...emptyDb(),
      families: [{ id: 'f1', name: 'כהן', createdAt: '2020-01-01', members: [{ id: 'm1', first: 'רוני', birth: birthIso }] }],
    } as unknown as Db;
    expect(birthdaysToday(db, leapAdar2).some((b) => b.first === 'רוני')).toBe(true);
  });
});
